...
- Show the context-usage indicator on first entry into a session instead of only after navigating away and back: the Codex bridge now consumes and persists `thread/tokenUsage/updated` (using fresh input tokens normally and the compacted total when post-compaction input is zero) and carries `contextUsage` on bridge session summaries and `session-updated` events (including the sidecar poll diff), the in-memory session branch of the Session GET extracts usage from Codex `system/turn_complete` SDK messages (preferring the SDK-reported context window) instead of only scanning assistant messages, and the client no longer drops `session-updated` events that arrive before the initial GET returns — it fetches metadata instead — plus schedules a one-shot delayed metadata retry whenever the initial snapshot lacks `contextUsage`
