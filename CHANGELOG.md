...
### Added
- Pi can serve models from several LLM gateways at once. `PiProvider` previously resolved exactly one gateway (OpenCode's `resolveOpenCodeGatewayConfig`), so a model that exists only on a second aggregator — on this machine `claude-opus-5` and `claude-fable-5`, which live on `api.appintheloop.com` and are visible to OpenCode only because the user's `opencode.json` declares a custom `mafia` provider — could not appear in Pi's picker at all, and could not be routed even if typed by hand, because the generated Pi provider catalog only ever described that one gateway. Extra channels are declared through the shared `YEP_LLM_GATEWAYS` variable and fetched concurrently. Gateways commonly expose identical ids (both gateways on this machine list `claude-opus-4-8`, `claude-sonnet-5`, `gpt-5.6-*`, `MiniMax-M3`), so models from non-default channels are namespaced `<channelId>/<modelId>` with the channel label appended to the display name, while the default channel deliberately keeps bare ids and the unsuffixed `yep-anthropic`/`yep-openai-compatible` provider ids: every pre-existing Pi session, saved default, and Pi-persisted provider selection stores that form, and renaming it would break resume. Pi itself receives (provider, bare model id) pairs — its native model identity — and one generated provider is registered per channel/protocol. A model id with no channel prefix now resolves to any channel serving that bare model (preferring the default) instead of failing the session start, so existing sessions keep working. Only the default channel routes OpenAI-compatible traffic through the bridge's gateway proxy, because that proxy has a single hardcoded upstream and injects its own `X-Sub-Module`, so sending another channel through it would silently reach the wrong gateway; the accepted cost is that extra channels do not get the bridge's SSE chunk-coalescing workaround. A channel whose catalog request fails keeps its last successful models instead of disappearing, so one unreachable gateway cannot empty the picker. The bundled Pi extension now takes one credential per generated provider (`YEP_PI_LLM_API_KEYS`, with the legacy single-key variable still honoured) and drops any provider without a key rather than registering a model that fails at request time; the provider additionally scrubs `YEP_LLM_GATEWAYS` and each channel's key variable from the child environment, which Pi's bash tool inherits. The session reader re-attaches the channel namespace from the provider id Pi recorded, so a transcript can distinguish the same model id served by two gateways, and `resolveModelDisplayLabel` now hides only Yep's own default channels instead of matching a hardcoded list of OpenCode channel names that could never cover a dynamically declared gateway
...
- Extract the LLM gateway primitives into a provider-neutral `packages/server/src/llm-gateways/` module. "Resolve a base URL, an API key and an optional `X-Sub-Module` header, then fetch `/v1/models`" existed in four independent copies — `opencode-bridge/gateway-config.ts`, the Pi provider (which reused OpenCode's resolver and could therefore only ever see one gateway), `config.ts`'s session-title block (its own `getDefaultSessionTitleSubModule` plus a hardcoded ohmyrouter URL), and the gateway benchmark — so adding a second gateway meant editing every provider module. The shared module owns `LlmGatewayCredentials`/`LlmGatewayChannel`, `resolveDefaultLlmGatewayChannel` (behaviourally identical to `resolveOpenCodeGatewayConfig`: `OPENCODE_LLM_*` over `LLM_*`, ohmyrouter's `claude-code-internal` default), `resolveLlmGatewayChannels`, `fetchLlmGatewayModels`, and the protocol/header helpers; it is deliberately dependency-free so the bridge sidecars can use it without booting the server config. `gateway-config.ts` keeps its exact public shape (`OpenCodeGatewayConfig` is now an alias of the shared credentials type) and delegates, and OpenCode's own catalog still comes from the resident opencode server. Extra channels can be declared through `YEP_LLM_GATEWAYS`, either compactly (`aitl=https://api.example/v1|NEW_LLM_API_KEY|codex-internal|Label`, the last two fields optional) or as JSON; keys are referenced by environment variable name so the secret is not duplicated into a second variable that would reach logs and child environments, the default channel always sorts first so single-gateway callers keep their behaviour by taking `[0]`, and an invalid entry is skipped with a reported reason rather than failing startup — one typo in an extra gateway must not take the working default gateway down, and the server now warns about each skipped entry at startup so the failure is not silent (previously the only symptom would be a model missing from the picker). Pi now consumes the shared layer through the multi-channel routing described above, while OpenCode's catalog remains sourced from its resident server
...
- Give Pi its own permission-mode copy on the new-session form and the in-session mode selector. `useProviderPermissionModeConfig` branched on `provider === "opencode" || provider === "pi"`, so a Pi session was labelled "OpenCode Tool Approvals" and told the user the rules are "applied as per-session tool rules" — which describes OpenCode's server-side `permission` config, not Pi. Pi has no native permission policy at all (`PiProvider` passes `respectProviderDecision: false`, and `references/pi` has no permission/plan concept), so the enforcement is Yep's own mode policy applied to canonicalized Pi tool names (`bash`→Bash, `read`→Read, `edit`→Edit, `write`→Write, `ls`/`find`→Glob, `grep`→Grep); anything else Pi exposes, including extension tools, is not on the read-only allowlist and therefore always asks. The new copy names those tools explicitly and states that Pi's `plan` mode is only a stricter approval gate: no plan prompt is injected anywhere in the Pi path, so unlike Claude/Codex it does not change how the model behaves, it just keeps writes and commands behind a prompt
## [2026.8.4] - 2026-08-17
