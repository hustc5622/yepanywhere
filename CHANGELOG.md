...
### Changed
- 附件改为插入到输入框光标位置的内联 chip，而不是固定挂在输入框下方：在会话输入框和新建会话页选择 / 粘贴文件时，会在光标处写入 `@[文件名]` 标记（输入框内以高亮 chip 呈现），发送后用户气泡里也按同一位置内联渲染成可点击的附件 chip，从而保留“图片和文字的先后顺序”这一上下文信息。标记与服务端 prompt 清单（`User uploaded files:`）使用同一套文件名规范化规则，服务端行为不变；删除标记文本即移除对应附件，未被标记引用的附件（历史草稿、飞书等渠道消息）仍按原样显示在气泡下方，排队中的消息同样内联展示。
