...
### Fixed
- Show Codex file changes with workspace-relative paths instead of `[path hidden]`. Codex app-server reports absolute paths, so live projection previously rejected every normal `fileChange` path before the client could render it, while rollout refreshes could expose the same host-specific absolute path. Live events, app-server history, canonical journal replay, and JSONL normalization now share a fail-closed workspace projection: paths inside the trusted session `cwd` become relative, while outside-workspace, traversal, and cross-drive paths remain hidden or fingerprinted.
- Fix Codex session fast-path correctness without expanding storage/provider architecture: app-server history now uses direction- and session-locked opaque cursors with symmetric older/newer pagination and inclusive-anchor de-duplication; raw reasoning and all generated structured path fields are removed from REST payloads, real base64 image-generation/local-media pages use typed rollout fallback, and large command/MCP payloads are not copied into native envelopes. The catalog explicitly includes `cli`/`vscode`/`exec`/`appServer`, retains manifest-confirmed rows across TTL refreshes, removes invalidated manifest-only rows before recomposition, and supplies cheap metadata/mixed-provider routing. The client snapshot LRU uses bounded structural estimates and reference-preserving merges instead of serializing multi-megabyte payloads on the main thread.
...
- Stop reporting Pi sessions driven by an in-process host as idle. Yep decides external ownership from the process table: a definitive "no provider process has this project as its cwd" retired the session. That premise only holds for a provider run as a CLI. Pi is also embedded through its SDK — Pi Web runs `AgentSession` inside its own Next.js server — so the host process is named `next-server`, its cwd is the Pi Web install directory, and the probe answers `false` for every one of those sessions. Verified on a live install: a session whose JSONL was being appended that same second reported `ownership: {owner: "none"}` with no activity, and the probe returned `false` while the turn was streaming. The probe is now corroborating rather than authoritative for Pi: when it finds nothing, the append-only log decides, because a log that just grew and ends in an unfinished turn means something is driving the session right now. "Unfinished" is a trailing user prompt, a trailing tool result, or an assistant message that has not reached a terminal stop reason; the tail is classified from a bounded 256 KiB read so a multi-megabyte log costs the same as a small one. Periodic validation and the decay timeout consult the same evidence — validation so a log-evidenced session is not retired three seconds later, decay because a long thinking phase appends nothing for longer than the 30 s window — while log-only evidence expires after five minutes without another file change, so a host that dies mid-turn cannot leave ownership stuck forever. A positive process probe still wins on its own, so a CLI idling at its prompt keeps owning its session.
