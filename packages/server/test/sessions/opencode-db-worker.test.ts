...
} from "../../src/sessions/opencode-db.js";
...
beforeEach(async () => {
  ...
  workDir = await mkdtemp(join(tmpdir(), `opencode-dbw-${randomUUID()}-`));
...
describe("OpenCode SQLite worker", () => {
  ...
  it("keeps the event loop responsive while a slow query runs", async () => {
    ...
    try {
      const result = await queryOpenCodeRows(dbPath, SLOW_SQL, [], {
        label: "test.slow",
  ...
  it("cancels on timeout and stays usable afterwards", async () => {
    ...
    const timedOut = await queryOpenCodeRows(dbPath, SLOW_SQL, [], {
      label: "test.timeout",
      timeoutMs: 1,
    });
    ...
    const recovered = await queryOpenCodeRow(
      dbPath,
  ...
  it("falls back to inline execution when the worker is disabled", async () => {
    if (!sqlite) return;
    vi.stubEnv("OPENCODE_DB_WORKER", "false");
    const result = await queryOpenCodeRows(
      dbPath,
