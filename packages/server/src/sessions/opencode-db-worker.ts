/**
 * Dedicated worker thread for OpenCode SQLite reads.
 *
 * `node:sqlite`'s `DatabaseSync` is synchronous, so running OpenCode queries on
...
export type OpenCodeDbStatementMode = "all" | "get" | "run";

export interface OpenCodeDbStatement {
  sql: string;
  params?: readonly unknown[];
  /** Defaults to `all`. */
  mode?: OpenCodeDbStatementMode;
}

export interface OpenCodeDbWorkerRequest {
  dbPath: string;
...
export type OpenCodeDbWorkerFailure =
  | "sqlite-unavailable"
  | "query-failed"
  | "timeout"
  | "worker-unavailable";

export type OpenCodeDbWorkerResult =
  | { ok: true; results: unknown[]; durationMs: number }
  | {
      ok: false;
      reason: OpenCodeDbWorkerFailure;
      error?: unknown;
      durationMs: number;
    };
 ...
 * a multi-gigabyte OpenCode database is not pinned open between polls.
 */
...
interface PendingRequest {
  resolve(result: OpenCodeDbWorkerResult): void;
 ...
 *
 * OpenCode reads are dominated by one large shared database, so extra workers
 * would mostly contend on the same pages while multiplying resident memory.
 * Serializing also gives the timeout path a simple, correct cancellation
 * story: terminate the worker and respawn it.
 */
class OpenCodeDbWorkerPool {
  ...
  private spawn(): Worker | null {
    ...
    try {
      ...
      this.disabled = true;
      getLogger().warn(
        { err: error, event: "opencode_db_worker_unavailable" },
        "[opencode-db] Worker thread unavailable; falling back to inline queries",
      );
      return null;
    }
  }
  ...
  private settle(message: WorkerMessage): void {
    ...
    if (message.ok) {
      if (message.durationMs > pending.budgetMs) {
        getLogger().warn(
          {
            event: "opencode_db_query_budget_exceeded",
            label: pending.label,
            durationMs: message.durationMs,
            budgetMs: pending.budgetMs,
          },
          "[opencode-db] Query exceeded its budget",
        );
      }
      pending.resolve({
        ok: true,
  ...
  private failAll(reason: OpenCodeDbWorkerFailure, error?: unknown): void {
  ...
  async run(request: OpenCodeDbWorkerRequest): Promise<OpenCodeDbWorkerResult> {
    ...
    const startedAt = Date.now();
    return new Promise<OpenCodeDbWorkerResult>((resolve) => {
      const timer = setTimeout(() => {
        ...
        getLogger().warn(
          {
            event: "opencode_db_query_timeout",
            label: request.label,
            timeoutMs: request.timeoutMs,
          },
          "[opencode-db] Query timed out; restarting query worker",
        );
        this.failAll("worker-unavailable");
 ...
 */
export async function runIsolatedOpenCodeDbStatements(
  request: OpenCodeDbWorkerRequest,
): Promise<OpenCodeDbWorkerResult> {
  let worker: Worker;
  try {
  ...
  const startedAt = Date.now();
  try {
    return await new Promise<OpenCodeDbWorkerResult>((resolve) => {
      const timer = setTimeout(() => {
        resolve({
          ok: false,
          reason: "timeout",
      ...
      const finish = (result: OpenCodeDbWorkerResult) => {
        clearTimeout(timer);
        resolve(result);
      };
...
let pool: OpenCodeDbWorkerPool | null = null;

export function getOpenCodeDbWorkerPool(): OpenCodeDbWorkerPool {
  pool ??= new OpenCodeDbWorkerPool();
  return pool;
}

/** Terminate the shared worker. Primarily a test/shutdown hook. */
export async function shutdownOpenCodeDbWorker(): Promise<void> {
  const active = pool;
  pool = null;
