...
export interface UseSessionMessagesResult {
  ...
  /** Retain live configuration even when it arrives before session metadata. */
  updateSessionConfiguration: (configuration: SessionConfiguration) => void;
...
export function contextUsageFromStatus(
...
type CodexTurnTerminalStatus = "completed" | "failed" | "interrupted";
...
function getCodexTurnTerminal(message: Message): CodexTurnTerminal | null {
  if (message.type !== "system" || message.subtype !== "turn_complete") {
    return null;
  }
  const turnId =
    nonEmptyMessageIdentity(message.codexTurnId) ??
    nonEmptyMessageIdentity(message.turnId);
 ...
 */
export function useLegacySessionMessages(
  ...
  const initialSnapshot =
    preferDisplayHistory || !enabled ? null : initialSnapshotRef.current;
  ...
  const pendingSessionConfigurationRef = useRef<{
    projectId: string;
    sessionId: string;
  ...
  const processStreamMessage = useCallback(
    (incoming: Message, fromBufferedReplay = false) => {
      ...
      const terminal = getCodexTurnTerminal(incoming);
      const normalizedIncoming =
        terminal &&
        nonEmptyMessageIdentity(incoming.codexTurnId) !== terminal.turnId
          ? { ...incoming, codexTurnId: terminal.turnId }
          : incoming;
      ...
      const isReplay = normalizedIncoming.isReplay === true;
      const shouldApplyReplayDedupe =
        (fromBufferedReplay || isReplay) && isCodexProvider(provider);
      const incomingTimestampMs = getMessageTimestampMs(normalizedIncoming);
      ...
      setMessages((prev) => {
        ...
        if (shouldApplyReplayDedupe) {
          if (isEmptyAssistantContent(normalizedIncoming)) {
            return prev;
          }
          if (hasEquivalentJsonlMessage(prev, normalizedIncoming)) {
            return prev;
          }
        }

        const result = mergeStreamMessage(prev, normalizedIncoming);
  ...
  const applySessionMetadata = useCallback(
    (incoming: Session): Session => {
      let next = mergeSameSessionMetadata(sessionRef.current, incoming);
      const pending = pendingSessionConfigurationRef.current;
      if (pending?.projectId === projectId && pending.sessionId === sessionId) {
        next = { ...next, ...pending.configuration };
        pendingSessionConfigurationRef.current = null;
      }
  ...
  const updateSessionConfiguration = useCallback(
    (configuration: SessionConfiguration) => {
      ...
      pendingSessionConfigurationRef.current = nextPending;
      const current = sessionRef.current;
      if (current?.id === sessionId) {
        pendingSessionConfigurationRef.current = null;
        sessionRef.current = { ...current, ...nextPending.configuration };
        providerRef.current = sessionRef.current.provider;
        // Preserve other metadata updates queued in the same React batch.
        setSession((latest) =>
          latest?.id === sessionId
            ? { ...latest, ...nextPending.configuration }
            : latest,
        );
      }
    },
  ...
  const applySessionSnapshot = useCallback(
    (
      ...
      const nextSession = applySessionMetadata(data.session);
    ...
    [applySessionMetadata, updatePersistedTimestampWatermark],
  );
  ...
  const fetchSessionMetadata = useCallback(async () => {
    try {
      ...
      applySessionMetadata(data.session);
    } catch {
      // Silent fail for metadata updates
    }
  }, [applySessionMetadata, projectId, sessionId]);
  ...
  useEffect(() => {
    ...
    void loadLegacySnapshot().catch((err) => {
      if (!isCurrent) return;
      initialLoadCompleteRef.current = true;
      flushBuffer();
      setLoading(false);
  ...
  return {
    ...
    displayPage: null,
    displayQuestions: [],
    displayQuestionCoverage: "unavailable",
