...
这些现象不是一个单点故障。超大的会话响应首先拖慢或打断初始加载；加载失败又进入了违反 React Hook 调用顺序的渲染分支，使前端整体崩溃；生产切换时的静态资源回退策略会把缺失的 JavaScript chunk 伪装成 HTML 200；supervisor 异常退出后又不能准确表达或接管残留服务。完整修复必须覆盖整个链路。

### 2.1 已确认的现场证据

问题会话 `01a00ced-6cbb-7253-ac80-6d9b2dacbe5f` 的最近 100 条消息响应为 62,463,479 字节，且没有 HTTP 内容压缩。其中：

- 顶层 `messages[].content` 中的原始工具结果约 15.63 MB；
- 同一结果解析后的 `toolUseResult` 约 15.58 MB；
- API 又同时返回 `session.messages` 和顶层 `messages`，把消息列表整体重复一次；
- 9 个通用 `exec` 工具结果中包含 30 个 `data:image/*;base64` 图片，每个约 430–620 KiB；
...
React #300 的直接原因是 `SessionPageContent` 在部分 Hook 之后，根据错误或缺失会话提前返回，而后方仍存在其他 Hook。初次加载与失败渲染调用了不同数量的 Hook。对应时间段的后端日志只有 WebSocket 连接和断开，没有新的 resume/turn 请求，因此“回复发送失败”发生在前端崩溃之前或期间，并不是 AI 提供方拒绝了续聊。
...
进程现场显示：计划任务为 `Ready`，记录的 supervisor PID 已死亡，但 8022 主服务及桥接子进程仍存活，8023 维护端口未监听。当前状态判断过度依赖计划任务是否为 `Running`，不能准确识别“supervisor 丢失、服务仍存活”的降级状态。
...
- AI 回复完成后，用户的下一条消息能够调用既有 resume/queue 流程并到达后端；
...
- 所有修改最终通过一次统一生产构建部署；部署失败时自动恢复上一版本。

## 4. 方案选择

### 4.1 选择：边界投影、分层恢复、统一发布
...
- 用可验证的进程身份清单管理 supervisor 和子进程；
- 各阶段独立测试、独立提交，全部完成后只构建和部署一次。

这是影响面最可控的方案。传输优化不会改变模型上下文，页面恢复不会与服务端会话逻辑耦合，静态资源恢复也不依赖 React 已成功挂载。
...
客户端类型同步区分“浏览器 Session 元数据”和服务端完整 Session，不保留空的、过期的或兼容性的 `session.messages` 字段。
...
对达到最小有效体积的普通 HTTP JSON 启用 Hono 协商式 gzip/deflate。WebSocket upgrade、SSE 或其他流式响应不参与压缩中间件。未声明支持编码的客户端继续收到正确的未压缩 JSON。
...
`useSessionMessages` 提供一个可复用的初始加载函数和 `retryInitialLoad()`：
...
不在本阶段改变 provider resume 协议。验收重点是确认点击发送后后端日志出现对应请求，而不是只观察前端气泡。
...
- 动态 import 失败识别，包括 `Importing a module script failed` 和 `Failed to fetch dynamically imported module` 等浏览器变体。
...
生产配置显式启用维护服务，监听 `127.0.0.1:8023`。主服务仍使用现有 8022 配置。维护端口加入本机 status/readiness 检查，但不加入 FRP 配置，不允许外部公网访问。

## 10. 阶段五：统一构建、部署验证与自动回滚

### 10.1 发布边界

前四阶段分别实现、测试和提交，但不逐阶段部署。最终从无无关修改的已提交 HEAD 构建一个生产包，记录准确的 commit 和 build ID，避免连续发布导致旧标签页反复遇到 chunk 失配。
...
- 路由测试：`session.messages` 不存在，消息只返回一次。
- 大体积 fixture：序列化结果不含隐藏工具图片，并满足固定体积预算。
- 压缩测试：普通 JSON 协商压缩；WebSocket 和流式响应不受影响。
- 数据完整性：只读加载前后原 Session JSONL 字节不变；显式续聊后只允许追加新记录。

### 11.2 页面加载和发送

- `loading -> failed` 不触发 React #300。
- `failed -> retry -> ready` 能恢复 Session。
- failed/loading 状态不会提交消息。
- AI 完成后继续回复会调用 `resumeSession`。
- 运行中允许排队时调用 `queueMessage`。
- 发送失败后恢复草稿与附件。
...
- 人为终止 supervisor 后，验证新 supervisor 无中断接管健康子进程。
...
实施计划必须按以下顺序拆分，并在每阶段通过专项测试后再进入下一阶段：

1. Session 响应投影、客户端契约和 HTTP 压缩。
2. Hook 顺序、加载重试及发送失败恢复。
3. 静态资源 404、React 外版本检查和一次性刷新。
4. 进程清单 v2、状态模型、接管/清理及本地维护端口。
5. 部署备份保留、冒烟验证、自动回滚及最终端到端验收。

代码提交应与上述边界对应，便于审查和定位回归；生产环境只接收最终统一构建。
...
只有当五个阶段的自动化测试、真实问题会话检查、手机续发测试、进程恢复测试和事务式部署验证全部通过，并且生产 build ID 与最终提交一致时，才能声明本次问题修复完成。
