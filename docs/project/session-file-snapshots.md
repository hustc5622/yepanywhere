...
> 2026-09-21 开发状态更新：Codex／Pi 生产执行入口已移除本页描述的目录快照采集接线。旧记录仍以“旧版工作区观测”只读展示；新记录走工具操作账本。以下采集实现保留为旧机制说明和兼容测试，不代表新执行仍会扫描目录。详见[工具驱动开发计划](./2026-09-21-session-tool-file-changes-development-plan.md)及[逐步验收记录](./2026-09-21-session-tool-file-changes-acceptance.md)。
...
原生 turn／entry ID 绑定后，每 15 秒进行一次执行中检查点采集；上一次采集完成后才安排下一次，
结束、拒绝或断线时清理定时器。检查点始终与执行前基线比较，记录 `status: active` 与同一
`captureId`。文件列表只采用同一 captureId 的最新观测，最终记录优先于执行中记录，
因此不会重复计数，也不会把已撤销的临时修改永久留在列表中。旧检查点 ID 仍可显式读取。
内容和覆盖信息均未改变时复用已有快照，避免空闲长轮次持续写入相同元数据。
当前仍非工具级归属，也未接入文件 watcher；加上前端轮询间隔，执行中的变化通常在约 20 秒加扫描耗时内出现。
...
- 默认单文件 2 MiB、一次实际采集内容总量 64 MiB、枚举上限 20,000 条目。
  预算分配默认使用 `source-first`：先源码、文档和配置，后常见日志／数据产物目录与二进制媒体。
  优先级仅影响采集顺序，不额外删除文件范围；不足的部分仍标记 byte-budget。
  排序策略保存在 policy 内，旧的字母排序快照不会与新策略混用来计算精确 diff。
...
文件索引的 `coverageReasons` 说明总量预算、单文件大小、枚举数量、不可采集路径、执行窗口或工作区不匹配等覆盖限制。全局覆盖不足不代表已保存文件内容不完整：进入 `changes` 的文件已具备确定的前后状态，其版本和 diff 的完整性只受执行窗口覆盖影响。

列表行数来自最近一次保存执行的 before/after 文本差异，与默认打开的版本一致，不跨执行累计，也不读取当前磁盘。新增与删除分别显示 `+N / -N`（包含零）；二进制、内容损坏或统计超时不伪造零值。只计算返回的前 500 个文件，并复用索引缓存，单个 diff 限时 25ms、每次索引合计计算预算 250ms。
