...
const BASELINE_FILE = join(PROTOCOL_ROOT, "baseline.ts");
const CODEX_COMMAND = process.env.CODEX_PATH?.trim() || "codex";
...
function parseMode(argv) {
  const supported = new Set(["--check", "--sync"]);
  const unknown = argv.filter((arg) => !supported.has(arg));
  if (unknown.length > 0) {
    throw new Error(`Unknown argument(s): ${unknown.join(", ")}`);
  }
  if (argv.includes("--check") && argv.includes("--sync")) {
    throw new Error("--check and --sync cannot be combined");
  }
...
function runCommand(command, args) {
  ...
  if (result.status !== 0) {
    const output = [result.stdout?.trim(), result.stderr?.trim()]
      ...
      .join("\n");
    throw new Error(
      `Command failed: ${command} ${args.join(" ")}${result.error ? `\n${result.error.message}` : ""}${output ? `\n${output}` : ""}`,
    );
  }
...
function getCodexVersion() {
  const output = runCommand(CODEX_COMMAND, ["--version"]);
...
function buildIndexContent() {
  ...
  lines.push('export { CODEX_PROTOCOL_BASELINE } from "./baseline.js";');
  lines.push("");
  return lines.join("\n");
}

function buildBaselineContent(manifest) {
  return [
    "// AUTO-GENERATED by scripts/update-codex-protocol.mjs. Do not edit.",
    "export const CODEX_PROTOCOL_BASELINE = {",
    `  codexVersion: ${JSON.stringify(manifest.codex.version)},`,
    `  stableSchemaHash: ${JSON.stringify(manifest.capabilityProfiles.stable.schemaHash)},`,
    `  experimentalSchemaHash: ${JSON.stringify(manifest.capabilityProfiles.experimental.schemaHash)},`,
    "} as const;",
    "",
  ].join("\n");
}
...
function writeExpectedCodexVersion(rootPackage, version) {
  rootPackage.yepAnywhere ??= {};
  rootPackage.yepAnywhere.codexCli ??= {};
  rootPackage.yepAnywhere.codexCli.expectedVersion = version;
  writeFileSync(
    ROOT_PACKAGE_FILE,
...
function main() {
  ...
  try {
    ...
    if (mode === "sync" && String(error).includes("ENOENT")) {
      console.warn(
        `Skipping Codex protocol sync because ${CODEX_COMMAND} is not installed.`,
      );
      return;
    }
    throw error;
  }
  if (mode === "check" && codexVersion.version !== expectedVersion) {
    throw new Error(
      `Codex version mismatch: installed ${codexVersion.version}, expected ${expectedVersion ?? "<unset>"} in package.json. Run \`pnpm codex:protocol:sync\` to update the checked baseline.`,
    );
  }
  if (mode === "sync" && expectedVersion) {
    const comparison = compareCodexVersions(
      codexVersion.version,
      expectedVersion,
    );
    if (comparison === null) {
      throw new Error(
        `Cannot compare installed Codex ${codexVersion.version} with checked baseline ${expectedVersion}; use \`pnpm codex:protocol:update\` for an explicit regeneration.`,
      );
    }
    if (comparison < 0) {
      console.warn(
        `Skipping Codex protocol sync: installed ${codexVersion.version} is older than checked baseline ${expectedVersion}.`,
      );
      return;
    }
  }
  const currentManifest = existsSync(MANIFEST_FILE)
    ? readJson(MANIFEST_FILE)
    : null;
  if (
    mode === "sync" &&
    codexVersion.version === expectedVersion &&
    currentManifest?.codex?.version === codexVersion.version &&
    existsSync(BASELINE_FILE)
  ) {
    const coverageCounts = validateCoverage(currentManifest);
    console.log(
      `Codex ${codexVersion.version} protocol baseline is already synchronized.`,
    );
    console.log(
  ...
  const expectedBaselineFile = join(expectedRoot, "baseline.ts");
  ...
  try {
    ...
    writeFileSync(
      expectedBaselineFile,
      buildBaselineContent(manifest),
      "utf-8",
    );
    runCommand("corepack", [
      "pnpm",
    ...
    if (mode === "check") {
      ...
      const baselineMatches = compareFile(BASELINE_FILE, expectedBaselineFile);
      const drift =
      ...
      if (drift) {
        ...
        if (!baselineMatches) console.log("  ~ baseline.ts");
    ...
    copyFileSync(expectedBaselineFile, BASELINE_FILE);
    if (expectedVersion !== codexVersion.version) {
      writeExpectedCodexVersion(rootPackage, codexVersion.version);
    }
